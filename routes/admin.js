...
router.get("/admin/settings", csrfProtection, restrict, (req, res) => {
  res.render("settings", {
    ...
    googleAnalytics:
      typeof req.app.config.googleAnalytics !== "undefined" ? escape.decode(req.app.config.googleAnalytics) : null,
    csrfToken: req.csrfToken(),
  });
});
...
router.post("/admin/searchall", restrict, async (req, res, next) => {
  ...
  if (Object.keys(customerQuery).length > 0) {
    customers = await db.customers.find(customerQuery).limit(limitReturned).sort({ created: 1 }).toArray();
  }

  // Get orders
  if (Object.keys(orderQuery).length > 0) {
    orders = await db.orders.find(orderQuery).limit(limitReturned).sort({ orderDate: 1 }).toArray();
  }

  // Get products
  if (Object.keys(productQuery).length > 0) {
    products = await db.products.find(productQuery).limit(limitReturned).sort({ productAddedDate: 1 }).toArray();
  }
