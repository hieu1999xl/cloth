...
const { getId, hooker, clearSessionValue, getImages, addSitemapProducts, getCountryList } = require("../lib/common");
...
router.post("/product/updatecart", async (req, res, next) => {
  ...
  if (!product) {
    res
      .status(400)
      .json({ message: "There was an error updating the cart", totalCartItems: Object.keys(req.session.cart).length });
    return;
  }
...
router.post("/product/removefromcart", async (req, res, next) => {
  ...
  return res
    .status(200)
    .json({ message: "Sản phẩm đã được loại bỏ ", totalCartItems: Object.keys(req.session.cart).length });
});
...
router.get("/search/:searchTerm/:pageNum?", (req, res) => {
  ...
  Promise.all([paginateProducts(true, db, pageNum, { _id: { $in: lunrIdArray } }, getSort()), getMenu(db)])
    .then(([results, menu]) => {
      // If JSON query param return json instead
