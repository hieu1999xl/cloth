...
router.post("/checkout_action", async (req, res, next) => {
  ...
  try {
    ...
    indexOrders(req.app).then(() => {
      ...
      req.session.message =
        "Your order is being received and is in the process of being processed. Below is the order information";
      req.session.paymentEmailAddr = newDoc.ops[0].orderEmail;
      req.session.paymentApproved = true;
      req.session.paymentDetails = `<p><strong>Code orders: </strong>${newId}</p>
        <p><strong>Time: </strong>${orderDoc.orderDate}</p>
      ...
      sendEmail(
        ...
        `Notice of successful order from Ogani`,
