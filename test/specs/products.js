...
test("[Success] Filter products", async (t) => {
  const res = await g.request.get("/admin/products/filter/backpack").set("apiKey", g.users[0].apiKey).expect(200);
...
test("[Success] Add a product", async (t) => {
  ...
  const res = await g.request.post("/admin/product/insert").send(product).set("apiKey", g.users[0].apiKey).expect(200);

  // Check the returned message
  t.deepEqual(res.body.message, "Sản phẩm được tạo thành côgn");
});
...
test("[Success] Add a product with incorrect GTIN", async (t) => {
  ...
  const res = await g.request.post("/admin/product/insert").send(product).set("apiKey", g.users[0].apiKey).expect(400);
...
test("[Success] Update a product", async (t) => {
  ...
  const res = await g.request.post("/admin/product/update").send(product).set("apiKey", g.users[0].apiKey).expect(200);
...
test("[Fail] Update a product - Duplicate permalink", async (t) => {
  ...
  const res = await g.request.post("/admin/product/update").send(product).set("apiKey", g.users[0].apiKey).expect(400);
