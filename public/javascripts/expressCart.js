...
function updateCartDiv() {
    $.ajax({
        ...
        .done(function (result) {
            ...
            var productHtml = '';
            var totalAmount = numeral(session.totalCartAmount).format('');

            // Work out the shipping
            var shippingTotalAmt = numeral(session.totalCartShipping).format('');
            var shippingTotal = `${session.shippingMessage} :<strong id="shipping-amount">${shippingTotalAmt} ${result.currencySymbol}</strong>`;
            if (session.totalCartShipping === 0) {
                shippingTotal = `<span id="shipping-amount">${session.shippingMessage}</span>`;
            }

            var discountTotalAmt = numeral(session.totalCartDiscount).format('');
            var discountTotal = '';
            ...
            if (cart) {
                ...
                Object.keys(cart).forEach(function (cartId) {
                    ...
                    // Setup the product
                    var productTotalAmount = numeral(item.totalItemPrice).format('');
                    var variantHtml = '';
                    if (item.variantId) {
            ...
            <div class="d-flex flex-row">
                <div class="cart-contents-shipping col-md-12 no-pad-right">
                    ...
                    <div class="text-right">
                        Total:
                        <strong id="total-cart-amount">${totalAmount} ${result.currencySymbol}</strong>
                    </div>
                </div>
            </div>`;
